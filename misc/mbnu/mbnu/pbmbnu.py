...
def calc_mean(data_dir, seg_file, label_file, sessid_file=None):
    if sessid_file == None:
        sessid_list = []
    ...
    for sess in sessid_list:
        ...
        #meants = []
        meants = np.zeros(len(label_list[1:]))
        for label in label_list[1:]:
            ...
            meants[label-1] = dat_tmp.mean()
        
        # For plotting each meants plot
        plt.plot(range(len(meants)), meants)
        plt.title('Mean Measure for Each Node')
        #('Distributions for Each Node')
        plt.xlabel('#Node')
        plt.ylabel('Gray matter Volume')
        plt.savefig(outfig, dpi=600)
        
        np.savez(outdata, meants=meants)
...
def calc_corr_mat(data_dir, sessid_file=None):
    """
    """
    if sessid_file == None:
        sessid_list = []
    ...
    databank = 'meants.npz'
    mat_fig = 'corr_mat.png'
    mat_dat = 'corrmat'
    rmat_fig = 'corr_mat_reordered.png'
    rmat_dat = 'corr_mat_reordered'
...
def pbmbnu(data_dir, seg_file, label_file, sessid_file=None):
    """
    population-based morphological brain network utilities
    """
    print data_dir, seg_file, label_file, sessid_file
    calc_mean(data_dir, seg_file, label_file, sessid_file)
    #calc_corr_mat(data_dir, sessid_file)
    return 1
